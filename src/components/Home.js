...
function Home() {
      ...
      return {
        ...
        content: (
          <div>
            <br />
            <p>
              <b>Request Key:</b> {res.reqKey}
            </p>
            <p>
              <b>Block Height:</b> {res.metaData.blockHeight}
            </p>
            <p>
              <b>Block Hash:</b> {res.metaData.blockHash}
            </p>
            <p>
              <b>Result:</b> {JSON.stringify(res.result.error.message)}
            </p>
          </div>
        ),
        hidden: false,
      ...
      return {
        ...
        content: (
          <div>
            ...
            <p>
              <b>Result:</b> {JSON.stringify(res.result.data)}
            </p>
            <p>
              Check Your TX{" "}
              <a
                href={`https://explorer.chainweb.com/${network}/tx/${res.reqKey}`}
              >
                <b>here</b>
              </a>
            </p>
  ...
  const status = () => {
    const requestContent = {
      ...
      1: {
        header: "Sign your Wallet",
        content: (
          <p>
            1. In the Chainweaver popup, press 'Next'. <br />
            2. Select public key to sign the transaction.
            <br />
            3. Then press 'Next' and 'Submit'"
          </p>
        ),
        hidden: false,
      },
      ...
      7: {
        header: "Preview: Failure",
        content: JSON.stringify(localRes),
        error: true,
        hidden: false,
      },
      8: {
        header: "Preview: Success",
        content: JSON.stringify(localRes),
        error: false,
        hidden: false,
      },
  ...
  return (
    <div className="App">
      ...
      <header className="App-header">
        ...
        <h1>Kadena {network === "mainnet" ? "" : "Testnet"} Chain Relay</h1>
        <h5>
          Create and manage Kadena Chain Relay Bonds{" "}
          {network === "mainnet" ? "" : "on Testnet"}
        </h5>
        ...
        >
          ...
          >
            <label
              style={{ color: "#18A33C", marginBottom: 5, textAlign: "left" }}
            >
              Create a New Bond
            </label>
            <Input
              ...
              action={
                <SUIButton
                  disabled={
                    key.length !== 64 ||
                    publicKeys.indexOf(key) !== -1 ||
                    (false &&
                      wallet.account.guard &&
                      wallet.account.guard.keys.includes(key))
                  }
                  icon="add"
                  onClick={() => {
          ...
          <List
            celled
            style={{ overflowX: "auto", width: 560, margin: "auto" }}
          >
            {publicKeys.length > 0 ? (
                  ...
                  <div>
                    {userBonds.map((bond) => {
                      ...
                      if (bond.bond.activity && pact.poolInfo.activity) {
                        achieved =
                          bond.bond.activity.int >= pact.poolInfo.activity.int;
                      }
                      return (
