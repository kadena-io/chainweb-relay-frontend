...
const BondInfo = (props) => {
  ...
  const bondDate = bond.bond.date
    ? bond.bond.date.time
      ? bond.bond.date.time
      : bond.bond.date.timep
    : "";

  const renewable =
    bond.bond.date && unlock && addDay(bondDate, lockup)
      ? new Date(addDay(bondDate, lockup)) < new Date() &&
        new Date() < new Date(addDay(bondDate, lockup + unlock))
      : false;
  const unbondable =
    bond.bond.date && unlock && addDay(bondDate, lockup + unlock)
      ? new Date(addDay(bondDate, lockup + unlock)) < new Date()
      : false;
  const active =
    bond.bond.date && unlock && addDay(bondDate, lockup)
      ? new Date(addDay(bondDate, lockup)) > new Date()
      ...
      >
        ...
        <div className="btable">
          <table className="btable">
            <tbody>
              <tr className="bitem">
                ...
                <td colSpan="3" className="blong bname">
                  {bond.key.length > 64
                    ? bond.key.slice(0, 30) + "..." + bond.key.slice(45)
                    : bond.key}
                </td>
              </tr>
              <tr className="bitem">
              ...
              <tr className="bitem">
                ...
                <td className="bvalue">
                  {bond.bond.activity && bond.bond.activity.int}
                </td>
                <td className="blabel">APY</td>
                <td className="bvalue">
                  {bond.bond.rate && (bond.bond.rate * 100 * 365).toFixed(2)}%
                </td>
              </tr>
              <tr className="bitem">
                <td className="blabel">Date</td>
                <td className="bvalue">
                  {bond.bond.date && bondDate.slice(0, 19)}
                </td>
                <td className="blabel">Renew</td>
                <td className="bvalue">
                  {bond.bond.date &&
                    unlock &&
                    addDay(bondDate, lockup).slice(0, 19)}
                </td>
              </tr>
              <tr className="bitem">
                ...
                <td className="bvalue">
                  {bond.bond.date &&
                    unlock &&
                    addDay(bondDate, lockup + unlock).slice(0, 19)}
                </td>
                <td className="blabel">Status</td>
                <td className="bvalue">
                  {bond.bond.terminated
                    ? "terminated"
                    : active
                    ? "active"
                    : "inactive"}
          ...
          <SimpleSign
            ...
            bondInfo={bond.bond}
          />
          <SimpleSign activity="Unbond" bond={bond.key} bondInfo={bond.bond} />
          <Rotate
            disabled={bond.key === ""}
            activity="Rotate"
            bond={bond.key}
